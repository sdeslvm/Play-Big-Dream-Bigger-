...
protocol PlayBigGradientProviding {
...
final class PlayBigGradientContainerView: UIView, PlayBigGradientProviding {
    ...
    func createGradientLayer() -> CAGradientLayer {
        ...
        layer.colors = [
            UIColor(playBigHex: "#1BD8FD").cgColor,
            UIColor(playBigHex: "#0FC9FA").cgColor
        ]
...
extension UIColor {
    ...
    convenience init(playBigHex hexString: String) {
...
struct PlayBigWebViewBox: UIViewRepresentable {
    // MARK: - Свойства
    
    @ObservedObject var loader: PlayBigWebLoader
    
    // MARK: - Координатор
    
    func makeCoordinator() -> PlayBigWebCoordinator {
        PlayBigWebCoordinator { [weak loader] status in
    ...
    private func setupContainerView(with webView: WKWebView) {
        let containerView = PlayBigGradientContainerView()
