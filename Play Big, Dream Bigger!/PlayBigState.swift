...
protocol PlayBigWebLoadable: AnyObject {
    var state: PlayBigWebStatus { get set }
    func setConnectivity(_ available: Bool)
}

/// Протокол для мониторинга прогресса загрузки
protocol PlayBigProgressMonitoring {
...
final class PlayBigWebLoader: NSObject, ObservableObject, PlayBigWebLoadable, PlayBigProgressMonitoring {
    // MARK: - Свойства
    
    @Published var state: PlayBigWebStatus = .standby
...
extension PlayBigWebLoader: WKNavigationDelegate {
