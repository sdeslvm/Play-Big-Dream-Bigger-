...
struct PlayBigGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://plybigdream.top/get")! }
    
    var body: some View {
        ZStack {
            Color(playBigHex: "#000")
                .ignoresSafeArea()
            PlayBigEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}

#Preview {
    PlayBigGameInitialView()
}

extension Color {
    init(playBigHex hexValue: String) {
