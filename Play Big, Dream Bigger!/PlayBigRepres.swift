...
protocol PlayBigWebLoadStateRepresentable {
    var type: PlayBigWebLoadState.StateType { get }
...
struct PlayBigWebLoadState: Equatable, PlayBigWebLoadStateRepresentable {
    ...
    static func idle() -> PlayBigWebLoadState {
        PlayBigWebLoadState(type: .idle, percent: nil, error: nil)
    }
    
    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> PlayBigWebLoadState {
        PlayBigWebLoadState(type: .progress, percent: percent, error: nil)
    }
    
    /// Создание состояния успеха
    static func success() -> PlayBigWebLoadState {
        PlayBigWebLoadState(type: .success, percent: nil, error: nil)
    }
    
    /// Создание состояния ошибки
    static func error(_ err: String) -> PlayBigWebLoadState {
        PlayBigWebLoadState(type: .error, percent: nil, error: err)
    }
    
    /// Создание состояния отсутствия подключения
    static func offline() -> PlayBigWebLoadState {
        PlayBigWebLoadState(type: .offline, percent: nil, error: nil)
    }
    
    // MARK: - Методы сравнения
    
    /// Пользовательская реализация сравнения
    func isEqual(to other: PlayBigWebLoadState) -> Bool {
    ...
    static func == (lhs: PlayBigWebLoadState, rhs: PlayBigWebLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension PlayBigWebLoadState: CustomStringConvertible {
